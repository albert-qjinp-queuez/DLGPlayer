...
@interface DLGOpenGLViewRenderer(){
...
@implementation DLGOpenGLViewRenderer
...
- (void)initProjection {
    [DLGOpenGLViewRenderer ortho:_mat4Projection];
}
...
- (void)render:(DLGPlayerVideoFrame *)frame{
    ...
    if (self.keepLastFrame) self.lastFrame = frame;
}
...
- (void)createGLProgram {
    ...
    GLuint vertexShader = [DLGOpenGLViewRenderer loadShader:GL_VERTEX_SHADER withFile:vertexShaderFile];
    NSString *fragmentShaderResource = self.isYUV ? @"DLGPlayerYUVFragmentShader" : @"DLGPlayerRGBFragmentShader";
    NSString *fragmentShaderFile = [bundle pathForResource:fragmentShaderResource ofType:@"glsl"];
    GLuint fragmentShader = [DLGOpenGLViewRenderer loadShader:GL_FRAGMENT_SHADER withFile:fragmentShaderFile];
...
- (void)updatePosition {
    const float cw = self.contentSize.width;
    const float ch = self.contentSize.height;
    if (cw == 0 || ch == 0) {
        _ratio[0] = _ratio[1] = 1;
        _vec4Position[0] = -1; _vec4Position[1] = -1;
        _vec4Position[2] =  1; _vec4Position[3] = -1;
    ...
    } else if (self.view.contentMode == UIViewContentModeScaleToFill) {
...
- (void)updateRotationMatrix {
    if (_shouldRotate) {
        if (_isFlipVertical) { [DLGOpenGLViewRenderer flipVertical:_mat3Rotation]; }
        else { [DLGOpenGLViewRenderer rotate:self.rotation matrix:_mat3Rotation]; }
    }
    if (_shouldScale) {
        [DLGOpenGLViewRenderer ratio:(GLfloat)_backingWidth/(GLfloat)_backingHeight matrix:_mat3Ratio];
        [DLGOpenGLViewRenderer ratio:(GLfloat)_backingHeight/(GLfloat)_backingWidth matrix:_mat3ViewRatio];
        [DLGOpenGLViewRenderer scale:_scale matrix:_mat3Scale];
    }
}
